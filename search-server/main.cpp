...
public:
    ...
    int GetDocumentId(int index) const {
        if (index > id_in_order.size()) {
            throw out_of_range("Number out of range"s);
        }
...
private:
    ...
    static bool IsSomeSpecCarsInWord(const string& word) {
        return none_of(word.begin(), word.end(), [](char c) {return c >= 0 && c <= 31;});
    }

    static bool IsCorrectWord(const string& word) {
        if (word.empty() || word == "-"s || (word[0] == '-' && word[1] == '-')
            || !IsSomeSpecCarsInWord(word) || word.back() == '-') {
            return false;
        }
        return true;
    }
...
int main() {
    ...
    try {
        ...
        cout << "отработал search_server\n";
        search_server.AddDocument(1, "пушистый кот пуый хвост"s, DocumentStatus::ACTUAL, { 7, 2, 7 });
        cout << "отработал AddDocument\n";
        search_server.AddDocument(2, "и в на"s, DocumentStatus::ACTUAL, { 1, 2 });
        cout << "отработал AddDocument\n";
        ...
        cout << "отработал FindTopDocuments\n";
